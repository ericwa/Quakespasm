...
// add an svc_spawnambient command to the level signon packet
	...
	for (i = 0; i < 3; i++)
		MSG_WriteCoord(&sv.signon, pos[i], sv.protocol, sv.protocolflags);
...
{
	MSG_WriteAngle (WriteDest(), G_FLOAT(OFS_PARM1), sv.protocol, sv.protocolflags);
}

static void PF_WriteCoord (void)
{
	MSG_WriteCoord (WriteDest(), G_FLOAT(OFS_PARM1), sv.protocol, sv.protocolflags);
}
...
static void PF_makestatic (void)
{
	...
	for (i = 0; i < 3; i++)
	{
		MSG_WriteCoord(&sv.signon, ent->v.origin[i], sv.protocol, sv.protocolflags);
		MSG_WriteAngle(&sv.signon, ent->v.angles[i], sv.protocol, sv.protocolflags);
	}
