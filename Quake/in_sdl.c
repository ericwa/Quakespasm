	...
	// mac os x mouse accel hack
	{
		...
		if (originalMouseSpeed == -1 && in_disablemacosxmouseaccel.value)
		{
			...
			if(mouseDev != 0)
			{
				...
				{
					Con_Printf("disabling mouse acceleration (previous: %g)\n", originalMouseSpeed);
					if(IOHIDSetAccelerationWithKey(mouseDev, CFSTR(kIOHIDMouseAccelerationType), -1.0) != kIOReturnSuccess)
					{
						Con_Printf("Could not disable mouse acceleration (failed at IOHIDSetAccelerationWithKey).\n");
						Cvar_Set ("in_disablemacosxmouseaccel", "0");
					}
				}
				else
				{
					Con_Printf("Could not disable mouse acceleration (failed at IOHIDGetAccelerationWithKey).\n");
					Cvar_Set ("in_disablemacosxmouseaccel", "0");
				}
			...
			{
				Con_Printf("Could not disable mouse acceleration (failed at IO_GetIOHandle).\n");
				Cvar_Set ("in_disablemacosxmouseaccel", "0");
			}
		}
	}
...
void IN_Deactivate (qboolean free_cursor)
{
	...
	// mac os x mouse accel hack
	{
		if(originalMouseSpeed != -1.0)
		{
			...
			if(mouseDev != 0)
			{
				Con_Printf("restoring mouse acceleration to previous: %g\n", originalMouseSpeed);
				if(IOHIDSetAccelerationWithKey(mouseDev, CFSTR(kIOHIDMouseAccelerationType), originalMouseSpeed) != kIOReturnSuccess)
					Con_Printf("Could not re-enable mouse acceleration (failed at IOHIDSetAccelerationWithKey).\n");
				IOServiceClose(mouseDev);				
			}
			else
			{
				Con_Printf("Could not re-enable mouse acceleration (failed at IO_GetIOHandle).\n");
			}
...
#ifdef MACOS_X_ACCELERATION_HACK
	Cvar_RegisterVariable(&in_disablemacosxmouseaccel, NULL);
#endif
