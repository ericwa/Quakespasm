...
void Host_Status_f (void)
{
	...
	for (j = 0, client = svs.clients; j < svs.maxclients; j++, client++)
	{
		...
		if (cmd_source == src_command)
			print_fn ("   %s\n", client->netconnection?NET_QSocketGetTrueAddressString(client->netconnection):"bot");
		else
			print_fn ("   %s\n", client->netconnection?NET_QSocketGetMaskedAddressString(client->netconnection):"bot");
	}
}
