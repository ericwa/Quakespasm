...
void SV_StartParticle (vec3_t org, vec3_t dir, int color, int count)
{
	...
	MSG_WriteCoord (&sv.datagram, org[0], sv.protocol, sv.protocolflags);
	MSG_WriteCoord (&sv.datagram, org[1], sv.protocol, sv.protocolflags);
	MSG_WriteCoord (&sv.datagram, org[2], sv.protocol, sv.protocolflags);
	for (i=0 ; i<3 ; i++)
	{
		v = dir[i]*16;
		if (v > 127)
...
// directed messages go only to the entity the are targeted on
	...
	for (i = 0; i < 3; i++)
		MSG_WriteCoord (&sv.datagram, entity->v.origin[i]+0.5*(entity->v.mins[i]+entity->v.maxs[i]), sv.protocol, sv.protocolflags);
}
...
// send an update
	...
	//
		...
		if (bits & U_ORIGIN1)
			MSG_WriteCoord (msg, ent->v.origin[0], sv.protocol, sv.protocolflags);
		if (bits & U_ANGLE1)
			MSG_WriteAngle(msg, ent->v.angles[0], sv.protocol, sv.protocolflags);
		if (bits & U_ORIGIN2)
			MSG_WriteCoord (msg, ent->v.origin[1], sv.protocol, sv.protocolflags);
		if (bits & U_ANGLE2)
			MSG_WriteAngle(msg, ent->v.angles[1], sv.protocol, sv.protocolflags);
		if (bits & U_ORIGIN3)
			MSG_WriteCoord (msg, ent->v.origin[2], sv.protocol, sv.protocolflags);
		if (bits & U_ANGLE3)
			MSG_WriteAngle(msg, ent->v.angles[2], sv.protocol, sv.protocolflags);
...
//
	if (ent->v.dmg_take || ent->v.dmg_save)
	{
		...
		for (i=0 ; i<3 ; i++)
			MSG_WriteCoord (msg, other->v.origin[i] + 0.5*(other->v.mins[i] + other->v.maxs[i]), sv.protocol, sv.protocolflags );
...
// a fixangle might get lost in a dropped packet.  Oh well.
	if ( ent->v.fixangle )
	{
		...
		for (i=0 ; i < 3 ; i++)
			MSG_WriteAngle (msg, ent->v.angles[i], sv.protocol, sv.protocolflags );
		ent->v.fixangle = 0;
	}
...
void SV_CreateBaseline (void)
{
	...
	//
		...
		for (i=0 ; i<3 ; i++)
		{
			MSG_WriteCoord(&sv.signon, svent->baseline.origin[i], sv.protocol, sv.protocolflags);
			MSG_WriteAngle(&sv.signon, svent->baseline.angles[i], sv.protocol, sv.protocolflags);
		}
